...
class DisplayManager:
    ...
    def display_ai_analysis(ai_analyzer, symbol, enhanced_metrics, news_articles):
        """Display AI analysis with form-based generation to minimize reloads"""
        ...
        with col1:
            if model_status['is_available']:
                ...
                # Use form to prevent page reload on button click
                with st.form(key=f"ai_form_{symbol}", clear_on_submit=False):
                    if st.session_state[analysis_key] is None:
                        st.info("Click below to generate professional AI investment analysis")
                        generate_clicked = st.form_submit_button("🧠 Generate Professional Analysis", type="primary")
                        
                        if generate_clicked:
                            # Generate analysis within the form context
                        ...
                        st.info("Analysis ready. Use controls below to manage.")
                        col_refresh, col_clear = st.columns(2)
                        with col_refresh:
                            refresh_clicked = st.form_submit_button("🔄 Generate New")
                            if refresh_clicked:
                                st.session_state[analysis_key] = None
                                st.experimental_rerun()
                        with col_clear:
                            clear_clicked = st.form_submit_button("🗑️ Clear Analysis")
                            if clear_clicked:
                                st.session_state[analysis_key] = None
